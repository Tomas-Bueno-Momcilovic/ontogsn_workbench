...
async function runAppPath(path) {
  if (typeof app.runPath === "function") {
    return app.runPath(path, { cache: "no-store", bust: true });
  }
  // fallback (matches your other panes)
  return app.run(String(path), null, { noTable: true });
}

async function runAppText(text) {
  if (typeof app.runText === "function") {
    return app.runText(text, { source: "terminal:inline" });
  }
  // fallback: run accepts raw SPARQL too (if yours does)
  return app.run(String(text), null, { noTable: true });
}
...
async function runQueryPath(path, { pretty = null } = {}) {
  ...
  try {
    ...
    const res = await runAppPath(path);
...
async function runQueryText(text) {
  ...
  try {
    const t0 = performance.now();
    const res = await runAppText(text);
    const dt = performance.now() - t0;
...
function showHelp() {
  ...
  appendLine("  run path data/queries/read_all_relations.sparql");
}
