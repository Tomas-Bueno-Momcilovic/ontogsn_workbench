...
function initDocView() {
  ...
  const btn = document.getElementById("tab-welcome");
  if (btn) {
    const loadWelcome = () => {
      const queryPath = btn.getAttribute("data-doc-query");
      const varHint   = btn.getAttribute("data-doc-var") || "";

      panes.activateLeftTab("tab-welcome"); // also initializes tabs

      return runDocQueryInto(root, queryPath, varHint);
    };

    // Clicking Welcome loads Welcome (and prevents document.js from hijacking)
    btn.addEventListener("click", (ev) => {
      ev.preventDefault();
      ev.stopPropagation(); // important!
      loadWelcome().catch(err => {
        console.error("[Welcome] error loading welcome", err);
        root.innerHTML =
          `<p class="doc-error">Error loading welcome: ${escapeHtml(err?.message || String(err))}</p>`;
      });
    });

    // Load it immediately on first page load
    loadWelcome().catch(err => {
      console.error("[Welcome] error loading initial welcome", err);
    });
  }
