...
function _emit(type, detail) {
    try { _bus?.emit?.(type, detail); } catch { }
...
function _stopStream() {
    ...
    if (_els.video) {
        try { _els.video.pause(); } catch { }
        try { _els.video.srcObject = null; } catch { }
    }
...
async function _setImageFromBlob(blob, {
    name = "image.png",
    source = "upload",
    status = "Loaded image.",
    stopCamera = true,
    ...
    // Stop camera preview if user loads an image (privacy + clarity)
    ...
    _setMeta({
        ...
        dims: (_current.width && _current.height) ? `${_current.width} × ${_current.height}` : "—",
    });
...
function _clearImage({ keepCamera = false } = {}) {
    ...
    if (_els.img) {
        try { _els.img.removeAttribute("src"); } catch { }
        _hide(_els.img, true);
    }
...
async function _startCamera() {
    ...
    try {
        ...
        _clearImage({ keepCamera: true }); // clears preview but keeps stream we just created
...
async function _captureFrame() {
    ...
    // If we already captured, "Retake" just returns to live preview
    ...
    // Show the still image (hide live preview)
...
export async function resume() {
    // Keep it simple: ensure buttons reflect the current state.
    _updateButtons();
}

export async function suspend() {
    // Privacy: stop webcam when user switches away from this pane.
    _stopStream();
    _updateButtons();
}
